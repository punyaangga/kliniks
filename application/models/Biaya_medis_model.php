...
class Biaya_medis_model extends CI_Model {
	...
	function _get($data = array())
    {
        ...
        if ($data['search']['value'] && !isset($data['all'])) {
            ...
            $q .= "WHERE (`nama_biaya_medis` LIKE '%". $s ."%' OR `biaya_medis` LIKE '%". $s ."%' OR `ket_biaya_medis` LIKE '%". $s ."%') AND `deleted_at` IS NULL ";
        } else{
	...
	function save($data = array())
	{
		...
		$q = '';
		if ($id == 0) {
			$q = "INSERT INTO `biaya_medis` (`created_at`, `nama_biaya_medis`, `biaya_medis`, `ket_biaya_medis`) VALUES (NOW(), '". $this->db->escape_str($f['nama_biaya_medis']) ."', '". $this->db->escape_str($f['biaya_medis']) ."', '". $this->db->escape_str($f['ket_biaya_medis']) ."');";
		} else{
			$q = "UPDATE `biaya_medis` SET `updated_at` = NOW(), `nama_biaya_medis` = '". $this->db->escape_str($f['nama_biaya_medis']) ."', `biaya_medis` = '". $this->db->escape_str($f['biaya_medis']) ."', `ket_biaya_medis` = '". $this->db->escape_str($f['ket_biaya_medis']) ."' WHERE `id` = '". $this->db->escape_str($id) ."';";
		}
